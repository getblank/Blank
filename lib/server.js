...
var nodePackageNames = ["blank-node-worker"];
...
function run() {
    let _cwd = path.resolve("bin") + path.sep;
    for (let packageName of goPackageNames) {
        let p = execFile(`./${packageName}`, null, {
            cwd: _cwd,
        });
        processLogger.bindStreams(p, packageName);
    }
    for (let packageName of nodePackageNames) {
        let cwd = "node_modules" + path.sep + packageName + path.sep;
        let p = spawn("node", ["."], {
            "cwd": cwd,
