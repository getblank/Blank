...
const writeConfigPart = (basePath, file, partFn) => {
...
module.exports = (appName, force = false) => {
    ...
    mkdirp(appName, (err, made) => {
        ...
        const demoStoresDir = path.join(made, "stores", "demo");
        mkdirp.sync(demoStoresDir);
        ...
        try {
            ...
            fs.writeFileSync(path.join(made, "stores", "settings.js"), settingsStore(appName), "UTF8");
        } catch (err) {
        ...
        try {
            writeConfigPart(demoStoresDir, "store.js", demoStore);
            writeConfigPart(demoStoresDir, "_itemHooks.js", demoHooks);
        } catch (err) {
            return console.error(err);
        }
