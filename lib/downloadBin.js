...
function downloadPackage(binPath, name) {
    ...
    return getLastVersionOfPackage(name)
        .then(version => {
            const downloadUrl = `https://github.com/getblank/${name}/releases/download/${version}/${name}-${process.platform}-amd64`;
            console.info(downloadUrl);
            return download(downloadUrl, path.join(binPath, name));
        })
        .then(() => {
