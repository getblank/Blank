...
function babelifyProps(props) {
    for (let propName of Object.keys(props)) {
        ...
        if (propDesc.loadComponent) {
            propDesc.loadComponent = babel.transform(propDesc.loadComponent, { presets: ["latest"] }).code;
            console.debug(propDesc.loadComponent);
        }
        if (propDesc.type === "virtual/client" && propDesc.load) {
            propDesc.load = babel.transform(propDesc.load, { presets: ["latest", "react"] }).code;
            console.debug(propDesc.load);
        }
        if (propDesc.props) {
...
function ship(config) {
    if (settings.output.indexOf("http") === 0) {
        ...
        mkdirp(confOutput, function (e) {
            ...
            fs.writeFile(outputFile, config, function (e) {
                if (e) {
