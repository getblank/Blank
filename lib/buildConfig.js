...
function createWatchers(configPath) {
    ...
    let libWatcher = chokidar.watch(settings.alibPaths, { persistent: true, ignoreInitial: true });
    libWatcher.on("change", function (_path) {
        clearTimeout(libTimer);
        libTimer = setTimeout(() => postLib(), 500);
    });

    let assetsWatcher = chokidar.watch(settings.aassetsPaths, { persistent: true, ignoreInitial: true });
...
function postAssets() {
    postZip(settings.assetsPaths.concat({ "folder": "blank", "file": "app.css", "data": less }), "assets", settings.output);
}
...
function updateLess(config) {
    if (config && config._commonSettings) {
        let _lessVars = config._commonSettings.lessVars || {};
...
function loadSettings(configPath) {
    ...
    settings.confPaths = [path.normalize(configPath + path.sep), path.normalize(defaultConfigPath + path.sep)];
    ...
    var bundlePath = path.join(packageDirname, "node_modules", "blank-web-app", "release") + path.sep;
    settings.assetsPaths.push({ "folder": "blank", "path": bundlePath });
    var fontsPath = path.join(packageDirname, "node_modules", "blank-web-app", "src", "fonts") + path.sep;
