...
if (help) {
    ...
    console.log("");
    console.log("Example:");
    console.log("   blank ./src -o http://httpbin.org/post -w");
    process.exit();
}
...
if (watch) {
    let timer = null;
    let configWatcher = _chokidar2.default.watch(_path2.default.normalize(configPath + _path2.default.sep), {
        persistent: true,
        ignoreInitial: true,
...
function prepareConfig() {
    ...
    loadFromFolder(config, _path2.default.resolve(__dirname, "./config/"));
    ...
    if (output.indexOf("http") === 0) {
        ...
        let srUrl = _url2.default.parse(output);
